jQuery(document).ready(function ($) {
  ...
  //fixed header
  if ($('#wrapper-header').hasClass('header__fixed') && !$('#page').hasClass('vertical')) {
    $('#wrapper-header').css('position', 'fixed');
  }
  var $header_wrapper = $('#wrapper-header').outerHeight();
  if ($('#wrapper-header').hasClass('header__fixed') && !$('#wrapper-header').hasClass('transparent') && !$('#page').hasClass('vertical')) {
    ...
    menuItems.forEach((item, index) => {
      item.addEventListener('keydown', (event) => {
        if (event.key === 'Tab' && !event.shiftKey) {
          // Check for Tab key without Shift
          if (index === menuItems.length - 1) {
            // If it's the last item in the menu
...
(function ($) {
  ...
  $('.photoswipe-image').click(function (event) {
    ...
    container.index = $(this).parent('figure').parent('div').parent('div').index();
  ...
  function initPhotoSwipeGallery() {
    ...
    $('.pswp__wrap')
      ...
      .each(function () {
        ...
        if (isHtml === true) {
          ...
          var content = card_content.length > 0 ? card_content[0].innerHTML : '';
          item = {
            html:
          ...
          if ($img.length > 0) {
            ...
            var lazyLoadSrc =
              $img.attr('data-src') || $img.attr('data-lazy-src') || $img.attr('data-original') || $img.attr('data-srcset') || $img.attr('data-lazy');
    ...
    const iconStyle = typeof minimalioLightboxSettings !== 'undefined' && minimalioLightboxSettings.iconStyle ? minimalioLightboxSettings.iconStyle : 'default';
...
(function ($) {
  ...
  $('.wp-block-gallery').each(function () {
    ...
    $pic.on('click', 'figure', function (event) {
      ...
      // Get icon style from localized PHP variable
      const iconStyle =
        typeof minimalioLightboxSettings !== 'undefined' && minimalioLightboxSettings.iconStyle ? minimalioLightboxSettings.iconStyle : 'default';
...
jQuery(document).ready(function ($) {
  ...
  function loadMoreOnScroll() {
    ...
    if (cardAuto) {
      ...
      setTimeout(function () {
        //this timeout simulates the delay from the ajax post
...
jQuery(document).ready(function ($) {
  ...
  $('input[type=radio][name=category]').change(function () {
    ...
    $('[data-filter]')
      .find($('[data-label]'))
      .not('[data-label="' + target + '"]')
      .removeClass('checked');

    $(this).toggleClass('checked');
    $('[data-filter]')
      .parent()
      .find($('[data-label="' + target + '"]'))
      .addClass('checked');
